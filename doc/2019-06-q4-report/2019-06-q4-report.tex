...
\begin{titlepage} % Suppresses displaying the page number on the title page and the subsequent page counts as page 1
    ...
    %------------------------------------------------
    \vspace{1cm}
    \textsc{\LARGE\bfseries UIUC-ARFC-2019-04} % Replace YYYY with the year, NN with report index
    \vspace{0.5cm}
    ...
    {\large June 28, 2019} % Date, change the \today to a set date if you want to be precise
    \vspace{0.5cm}
...
%------------------------------------------------
    ...
    Department of Energy ARPA-E MEITNER Program (award DE-AR0000983) and the 
    ...
    the state of Illinois. Blue Waters is a joint effort of the University of 
...
The University of Illinois, Urbana-Champaign (UIUC) is engaged in work to 
develop a fuel processing system that enables load-following in \glspl{MSR}, an 
important ability that allows nuclear power plants to ramp electricity 
production up or down to meet changing electricity demand. Nuclear reactions in 
\glspl{MSR} produce unwanted byproducts (such as xenon and krypton) that can 
adversely affect power production. In steady, baseload operation, these 
byproducts form and decay at the same rate. When electricity production is 
ramped down, however, the byproducts start to be produced at a greater rate 
than they decay, leading to a buildup within the reactor. When power production 
must be once again increased, the response rate is slowed by the time needed 
for the byproducts to reach their equilibrium level (determined by the 
radioactive decay half-life, which is on the order of hours). Thus, buildup of 
these unwanted byproducts resulting from ramping down inhibit proper load 
following for molten salt reactors. Fortunately, \glspl{MSR} transport fuel in 
a flowing molten salt fuel loop, which means that a section of the reactor, 
outside the core, can be leveraged for fuel processing and "cleanup." The team 
will determine the feasibility of removal of these unwanted byproducts and 
design a fuel reprocessing system, removing a major barrier to 
commercialization for molten salt reactors.

Toward this work, we initiated the Fuel Cycle Simulation task (Task 2) in August 2018 to more 
...
rykhlevskii_advanced_2018, rykhlevskii_arfc/saltproc:_2018}, was developed to 
represent the simplified online fuel salt processing of a \gls{MSBR}.
More recently, an advanced SaltProc version (SaltProc v0.2) was developed 
to generically simulate complex molten salt fuel reprocessing systems, including the \gls{TAP} 
system, 
...
document, we will discuss and demonstrate the capabilities of SaltProc v0.2 
...
\begin{enumerate}
	...
	\item Run Serpent 2 in parallel mode to perform depletion calculations 
	...
	\item Read the resulting depleted fuel composition file and store it in 
        ...
        \item Remove poisons from the fuel's isotopic composition by passing 
                ...
                SaltProc v0.2 used user-defined constant separation 
...
change criticality. Unmoderated salt flowing in the annulus between the core 
...
\begin{figure}[t] % replace 't' with 'b' to 
  ...
  \caption{The \gls{TAP} \gls{MSR} schematic core view showing moderator rods 
        in the \gls{BOL} position defined by their point design 
...
SaltProc v0.2  demonstration case without any modifications.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{table}[ht!]
        \centering
...
We simulated \gls{TAP} \gls{MSR} depletion in SaltProc v0.2 using  
...
rykhlevskii_arfc/saltproc:_2018}. The tool was designed to expand depletion 
capabilities of Serpent 2 for modeling liquid-fueled \gls{MSR} with an online 
fuel reprocessing system. SaltProc v0.1 uses HDF5  
\cite{the_hdf_group_hierarchical_1997} to store data and uses the PyNE Nuclear 
Engineering Toolkit \cite{scopatz_pyne:_2012} to parse Serpent 2 output. 
SaltProc v0.1 is an open-source Python package that uses a batch-wise approach 
...
We completely re-factored SaltProc v0.1 using \gls{OOP} to create a generic, 
...
\subsection{SaltProc v0.2 architecture}
The SaltProc v0.2 Python toolkit couples directly with the Serpent 2 input 
and output files, to allow the reprocessing system couples to depletion 
calculation. Existing PyNE interfaces are employed to parse Serpent output  
...
Figure~\ref{fig:saltproc_class} illustrates the SaltProc v0.2 class 
structure, consisting of 4 main classes:
\begin{figure}[ht!] % replace 't' with 'b' to \centering
  \includegraphics[width=1.07\textwidth]{saltproc_class_diagram.png}
  	  	\vspace{-0.35in}
  \caption{SaltProc v0.2 python package class diagram in UML notation with  
...
\subsection{SaltProc v0.2 flowchart}
Figure~\ref{fig:saltproc_flow} illustrates the online reprocessing simulation 
algorithm, coupling Serpent with SaltProc v0.2. To perform a depletion step, 
SaltProc v0.2 reads a user-defined Serpent template file. The template  
...
conditions. SaltProc v0.2 fills in the template file and runs the Serpent 
single-step depletion. After the depletion calculation, SaltProc v0.2 reads 
...
Serpent depletion run. SaltProc v0.2 also stores isotopic composition before 
...
\begin{figure}[ht!] % replace 't' with 'b' to \centering
  ...
  \includegraphics[width=1.03\textwidth]{saltproc_flowchart.pdf}
  	  	\vspace{-0.35in}
  \caption{SaltProc v0.2 python package flow chart.}
...
intensity. SaltProc v0.2 is an open-source tool (though having Serpent 
installed is required to use SaltProc v0.2) available on GitHub. It leverages 
unit and continuous tests crucial for sustainable development 
\cite{krekel_pytest:_2004}. Clickable documentation is available through 
Sphinx, a documentation generator, for ease of use \cite{brandl_sphinx:_2009}. 
In summary, the development of SaltProc v0.2 is focused on producing a 
...
The SaltProc v0.2 modeling and simulation tool is demonstrated for the 
...
\cite{leppanen_serpent_2013} are employed to represent the \gls{TAP} core. 
...
SVF &= \frac{V_F}{V_F+V_M} = \frac{1}{1+V_M/V_F}
	\intertext{where}
 	V_F &= \mbox{in-core fuel salt volume} \nonumber \\
 	V_M &= \mbox{in-core moderator volume} \nonumber \\
 	V_M/V_F &= \mbox{in-core moderator-to-fuel-salt ratio} \nonumber
\end{align}
The \gls{SVF} for model herein is 0.907268 which means the modeled core is 
under-moderated and has an intermediate/fast spectrum.
\begin{figure}[htp!] % replace 't' with 'b' to 
  \centering
...
scheme for the SaltProc v0.2 demonstration 
(figure~\ref{fig:demo-repro-scheme}).
\begin{figure}[htp!] % replace 't' with 'b' to 
  \centering
		  \includegraphics[width=0.93\textwidth]{demo_reprocessing_scheme.png}
  \caption{\gls{TAP} reprocessing scheme flowchart used for SaltProc v0.2 
...
efficiencies. We calculated these efficiencies for SalProc v0.2 input 
...
simulations, a 30-day timestep size will be applied.

\section{Results}
The SaltProc v0.2 online reprocessing simulation package is demonstrated for 
analyzing the \gls{TAP} \gls{MSR} neutronics and fuel cycle to find the equilibrium core composition and core depletion. The neutron population per cycle and the number 
...
and computational time. We accomplished it with a neutron population of 15,000, 
400 active cycles, and 200 inactive cycles. 
...
composition one time was approximately 9000 node-hours ($\approx$16 core-years).

\subsection{Effective multiplication factor}
Figures~\ref{fig:keff}, \ref{fig:keff-zoomed}, \ref{fig:keff-zoomed-2} demonstrate 
the effective 
multiplication factors  obtained using SaltProc v0.2 and Serpent. We obtained 
...
Figure \ref{fig:keff} clearly indicates taht the reactor went 
   subcritical too fast and further investigation is needed to overcome this issue. 
...
as adding new functionality to SaltProc v0.2.
...
converge with $>200$ inactive cycles 
...
\begin{figure}[htp!] % replace 't' with 'b' to 
  ...
  cycles number for the full core calculations with a neutron population  of $M=15,000$.}
  \label{fig:shannon}
\end{figure}
...
We checked the correctness of SaltProc v0.2 by comparing the mass of the 
...
the nickel filter is only 5\%. Figure~\ref{fig:xe-i} shows that SaltProc v0.2 
...
\gls{TAP} fuel reprocessing system simulated with SaltProc v0.2 allows 
...
\cite{betzler_fuel_2018}. Additionally, we will add in a SaltProc v0.2 
